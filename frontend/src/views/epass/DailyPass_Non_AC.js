...
function DailyPass_Non_AC() {
  useEffect(() => {

    const date = new Date();
    const year = date.getFullYear();
    const month = date.getMonth() + 1;
    const day_next = date.getDate() + 1;
  ...
  const handlepay = () => {
    ...
    var pass_id = "BMTC_DP_Non_AC_" + rand;
    ...
    const result = fetch(`http://localhost:9000/ePass_details/Daily_ePass_${busType}`, {
      method: "POST",
      headers: {
    ...
    window.location.href = "/admin/epass/viewDailyPass_Non_AC";
            ...
            <Card className="card-user">
              ...
              <Col className="pr-1" md="12">
                ...
                <Form.Text className="pr-1 form-control" onChange={({ target: { value } }) => {
                  ...
                  <option value="Non_Ac" selected>Non_AC</option>
...
export default DailyPass_Non_AC;
