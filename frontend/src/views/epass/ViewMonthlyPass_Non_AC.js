...
function ViewMonthlyPass_Non_AC() {
  ...
  const fetchdata = async () => {
    ...
    result.then((json) => {
      ...
      const passdetails = json.data[0].ePass_details.Monthly_ePass_Non_AC;
      const Pass_id = json.data[0].ePass_details.Monthly_ePass_Non_AC.Pass_id;

      const Doc_number = json.data[0].ePass_details.Monthly_ePass_Non_AC.DocNumber;

      const expiry = json.data[0].ePass_details.Monthly_ePass_Non_AC.ExpiryDate;
      const Purchase = json.data[0].ePass_details.Monthly_ePass_Non_AC.logindate;

      const Fare = json.data[0].ePass_details.Monthly_ePass_Non_AC.Fare;
     ...
     const expiryparsed = parseInt(expirydate[1]);
      const purchasedateparsed = parseInt(purchasedate[1]);
      const currentdate_parsed = parseInt(currentdate_[1]);

      if (currentdate_parsed >= expiryparsed) {
        console.log("Pass is expired");
        setValidPass(false);
      } else {
    ...
    // sessionStorage.setItem("user_pass_type", Monthly_ePass_Non_AC);
    sessionStorage.setItem("Verify_Pass_Type", "ScanVerifyPass_Monthly_ePass_Non_AC");
  ...
  return (
    ...
    <div className="container">
      {validPass && (<>
        <h1>My Monthly-Pass Non AC <img src={require("assets/img/bus_ticket_orange_small.png")} class="img-responsive" alt="..." />
</h1>
        <table class="table table-bordered table-responsive">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td colspan="4"><li>BasicCharge = 1400</li>
                <li> Service Charge = 30</li>
                <li>Tax = 20</li> <li>others = 50</li> </td>
              <td> </td>
...
export default ViewMonthlyPass_Non_AC
